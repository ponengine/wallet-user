...
import com.pon.user.dto.SearchDTO;
...
public class UserTransactionController {
	...
	@PostMapping("/showusersearch")
	public List<UserInfoDTO> showusersearch(@RequestBody SearchDTO searchDTO){
		List<UserInfoDTO> listdto = new ArrayList<>();
		Iterable<UserInfo> listtran = usernfoRepository.findBySearch(searchDTO.getStartDate(), searchDTO.getEndDate());
		for (UserInfo tranReport : listtran) {
			UserInfoDTO tranReports = new UserInfoDTO();
			try {
				tranReports = modelMapper.map(tranReport, UserInfoDTO.class);
